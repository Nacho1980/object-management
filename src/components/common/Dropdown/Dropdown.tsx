...
import { StyledSelect, ShortStyledSelect } from "./styles";
  ...
  return (
    <StyledSelect id={id} value={value} onChange={selectChange}>
      {options.map((item, index) => (
        <option key={`${id}-opt${index}`} value={item.value}>{item.label}</option>
      ))}
    </StyledSelect>
  )
}
 ...
 * @returns 
 */
 export const ObjectDropdown = ({id, value, onChange, objectArray}: ObjectDropdownProps) => {
  const selectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const newValue = parseInt(event.target.value);
    onChange(newValue);
  };
  return (
    <ShortStyledSelect id={id} value={value} onChange={selectChange}>
      {objectArray.map((obj, index) => (
        <option key={`${id}-opt${index}`} value={obj.id}>{elypsis(obj.name, MAX_LENGTH_OBJECT_NAME_DROPDOWN)}</option>
      ))}
    </ShortStyledSelect>
  )
}